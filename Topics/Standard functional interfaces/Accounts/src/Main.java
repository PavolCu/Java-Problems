...
class Main {
    ...
    private static final long EXCESSIVE_BALANCE = 100_000_000;

    public static void printFilteredAccounts(List<Account> accounts) {

        List<Account> nonEmptyAccounts = filter(accounts, account -> account.getBalance() > MINIMUM_BALANCE);
        List<Account> accountsWithTooMuchMoney = filter(accounts, account ->
                !account.isLocked() && account.getBalance() >= EXCESSIVE_BALANCE);
